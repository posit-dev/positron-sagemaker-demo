...
Study HLX-301: a randomised, placebo-controlled trial of an investigational
agent in moderate-to-severe disease, with a disease activity score (DAS) as the
efficacy measure.

The modelling target is early discontinuation, which is a *subject*-level
outcome and therefore lives on ``dim_subject``. The walkthrough joins it back to
aggregated visit history -- which is how this analysis is really done, and makes
for a more honest Athena query than selecting from one flat table.
"""
...
ENROLMENT_START = date(2023, 6, 1)
ENROLMENT_END = date(2024, 9, 30)
...
# region -> (countries, share of sites)
REGIONS = {
...
# --- Discontinuation log-odds model ----------------------------------------
# Tuned to land overall early discontinuation around 15%.
DISCONT_INTERCEPT = -2.05
DISCONT_BETAS = {
    "ae_count_z": 0.71,        # adverse events are the dominant driver
    "ecog_z": 0.34,            # worse performance status -> less able to continue
    "age_z": 0.26,
    "baseline_das_z": 0.29,    # higher disease burden -> more dropout
    "prior_lines_z": 0.22,
    "bmi_z": 0.08,
}
# Treatment arm sees more AE-driven withdrawal; placebo more lack-of-efficacy.
# The net effect on total discontinuation is close to neutral by design.
...
def build_dim_site(rng: np.random.Generator) -> pd.DataFrame:
    ...
    # Per-site retention effect: some sites simply keep subjects better.
...
def build_dim_subject(rng: np.random.Generator, sites: pd.DataFrame) -> pd.DataFrame:
    ...
    arm = rng.choice(ARMS, size=n)  # 1:1 randomisation
    ...
    # Enrolment date is bounded below by the site's activation date.
    span = (ENROLMENT_END - ENROLMENT_START).days
    wanted = pd.to_datetime(ENROLMENT_START) + pd.to_timedelta(
        rng.integers(0, span + 1, size=n), unit="D"
    )
    activation = pd.to_datetime(site["activation_date"])
    enrolment = pd.Series(np.maximum(wanted.values, activation.values))
    ...
    return pd.DataFrame(
        {
            ...
            "enrolment_date": enrolment.dt.date,
            "enrolment_date_key": enrolment.dt.strftime("%Y%m%d").astype("int32"),
...
def build_fct_visit_observations(
    ...
    """Return (fact table, dim_subject with discontinuation outcome attached).

    Adverse events are simulated first, because early AE burden is what drives
    the discontinuation decision -- so the causal order in the data matches the
    causal order in the trial.
    """
    n = len(subjects)
    n_visits = len(visits)
    ...
    # Per-visit adverse event probability: higher on treatment, front-loaded.
    visit_days = visits["scheduled_day"].to_numpy()
    recency = np.exp(-np.maximum(visit_days, 0) / 220.0)  # AEs cluster early
    ...
    # AE burden through Week 12 is the operational early-warning signal.
    ...
    # Discontinuers leave after a visit weighted toward the first year.
    ...
    # DAS trajectory: treatment improves more; non-response predicts dropout.
    treatment_effect = np.where(arm_is_treatment, -2.05, -0.62)
    # Subjects who discontinue respond less well -- a real, observable pattern.
    ...
    for v_idx in range(n_visits):
        ...
        # Visit windows slip a few days in the real world.
            ...
            pd.DataFrame(
                {
                    ...
                    # Adherence wanes before a subject formally withdraws, so
                    # missed visits are a genuine leading indicator rather than
                    # noise. Measured in a fixed early window (see the training
                    # query) this is prospective, not leakage.
